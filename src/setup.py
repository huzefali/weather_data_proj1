...
try:
    ...
    # user device access data update
    # User device have a access rights 'r' or 'rw'. 'Admin' has full rights
    # Reads access_user_device.csv one line at a time, splits them into the data fields and update in user collection
    with open(RELATIVE_CONFIG_PATH+ADD_ACCESS+'.csv', 'r') as access_user_device_fh:
        for access_user_device_row in access_user_device_fh:
            access_user_device_row = access_user_device_row.rstrip()
            if access_user_device_row:
                (username, device, acesss) = access_user_device_row.split(',')
            select_data = {'username' : username}
            access_list = {'did': device, 'atype': acesss}
            access_list = {'$push': {'alist' :access_list}}
            user_collection = weather_dbh[USER_COLLECTION]
            access_list_obj_id = user_collection.update_one(select_data, access_list)
